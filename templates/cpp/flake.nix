  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        remoteVersionUrl =
          "https://raw.githubusercontent.com/monadimi/nix-env/main/templates/cpp/version";
        remoteFlakeUrl =
          "https://raw.githubusercontent.com/monadimi/nix-env/main/templates/cpp/flake.nix";

        updateScript = pkgs.writeShellScriptBin "cpp-flake-self-update" ''
          set -euo pipefail
          ...
          read_remote_version_file() {
            curl -fsSL --max-time 5 "https://raw.githubusercontent.com/monadimi/nix-env/main/templates/cpp/version" 2>/dev/null               | tr -d "\r"               | head -n 1               | sed -e 's/[[:space:]]*$//'
          }
          ...
          curl -fsSL --max-time 10 "https://raw.githubusercontent.com/monadimi/nix-env/main/templates/cpp/flake.nix" -o "$tmp"
...
EOF
      ...
      in {
        ...
        apps.update-flake = {
          type = "app";
          program = "${updateScript}/bin/cpp-flake-self-update";
        };
